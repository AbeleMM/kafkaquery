package org.codefeedr.kafkatime.commands
...
import org.codefeedr.kafkatime.transforms.{Register, TimeOutFunction}
...
class QueryCommand extends Register {
    ...
    new Thread {
      override def run(): Unit = {
        ...
        while (true) {
          ...
          clientMap.put(s"""${newSocket.getInetAddress}:${newSocket.getPort}""",
                        newSocket)
        }
      }
    }.start()
  ...
  /**
    ...
    */
  def queryToKafkaTopic(outTopic: String,
                        ds: DataStream[Row],
                        kafkaAddress: String): Unit = {
